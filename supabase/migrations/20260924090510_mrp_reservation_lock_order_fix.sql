-- Serialize every operation for a product/location in the same lock order:
-- physical balance rows first, then the resource advisory lock, then reservation rows.
-- This prevents both cross-currency oversubscription and replacement deadlocks.
...
BEGIN
  ...
  -- Replacement must lock both its old resources and requested new resources
  -- before touching reservation rows. All callers use the same stable order.
  FOR v_resource IN
    SELECT resource.location_id, resource.producto_id
    FROM (
      SELECT p_location_id AS location_id,
             NULLIF(item.value ->> 'producto_id', '')::uuid AS producto_id
      FROM jsonb_array_elements(COALESCE(p_items, '[]'::jsonb)) AS item(value)
      WHERE NULLIF(item.value ->> 'producto_id', '') IS NOT NULL
        AND COALESCE(NULLIF(item.value ->> 'quantity', '')::numeric, 0) > 0
      UNION
      SELECT r.location_id, r.producto_id
      FROM public.inventory_reservations r
      WHERE p_replace
        AND p_plan_id IS NOT NULL
        AND r.weekly_plan_id = p_plan_id
        AND r.empresa_id = v_empresa_id
        AND r.status = 'ACTIVE'
    ) AS resource
    ORDER BY resource.location_id, resource.producto_id
  LOOP
    PERFORM 1 FROM public.inventory_balances b
      WHERE b.empresa_id = v_empresa_id
        AND b.location_id = v_resource.location_id
        AND b.producto_id = v_resource.producto_id
      ORDER BY b.cost_currency::text
      FOR UPDATE;

    PERFORM pg_advisory_xact_lock(hashtext(
      v_empresa_id::text || ':' || v_resource.producto_id::text || ':' || v_resource.location_id::text
    ));
  END LOOP;

  IF p_replace AND p_plan_id IS NOT NULL THEN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(COALESCE(p_items, '[]'::jsonb)) LOOP
    ...
    -- Resource rows and advisory lock were acquired above before reading totals.
...
BEGIN
  ...
  IF p_status = 'COMMITTED'
     ...
     AND jsonb_array_length(COALESCE(p_reserve_items, '[]'::jsonb)) > 0 THEN
    RAISE EXCEPTION 'No se puede reservar material sin ubicación central';
  END IF;
  ...
  IF p_status = 'COMMITTED' AND p_location_id IS NOT NULL THEN
    -- reserve_plan_stock locks resources before releasing/replacing old rows.
  ...
  ELSIF p_plan_id IS NOT NULL THEN
    -- DRAFT/CLOSED (or COMMITTED without a central location) release promises.
    UPDATE public.inventory_reservations
      SET status = 'RELEASED', released_at = now()
      WHERE weekly_plan_id = p_plan_id
        AND status = 'ACTIVE'
...
REVOKE ALL ON FUNCTION public.commit_production_plan_atomic(UUID, UUID, UUID, UUID, DATE, DATE, TEXT, TEXT, JSONB, UUID, UUID, JSONB, DATE, TEXT)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.commit_production_plan_atomic(UUID, UUID, UUID, UUID, DATE, DATE, TEXT, TEXT, JSONB, UUID, UUID, JSONB, DATE, TEXT)
  TO service_role;
...
BEGIN
  ...
  -- Balance row is already locked by UPDATE/DELETE. This shared resource lock
  -- serializes decreases across cost buckets; reserve_plan_stock uses the same
  -- order (balance rows, then advisory lock).
  PERFORM pg_advisory_xact_lock(hashtext(
    v_empresa_id::text || ':' || v_producto_id::text || ':' || v_location_id::text
  ));
