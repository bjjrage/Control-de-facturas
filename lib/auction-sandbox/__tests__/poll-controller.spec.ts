...
describe('PollController', () => {
  ...
  it('runMutation timeout settles visibly, resumes, and still refreshes once', async () => {
    const order: string[] = [];
    const ctl = new PollController(
      async () => {
        order.push('poll');
    ...
    await expect(
      ctl.runMutation(() => new Promise<string>(() => {}), 20, 'Iniciar subasta')
    ).rejects.toThrow(/Iniciar subasta tardó más de/);
