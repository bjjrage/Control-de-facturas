...
describe("Rodrigo ERP V5 capability boundary", () => {
  it("expone lecturas y separa las operaciones aprobables", () => {
    ...
    expect(getTool("get_supplier_invoice_overview")?.requiredRoles).toEqual(["administracion", "admin"]);
    for (const name of mutations) expect(getTool(name)?.riskLevel, name).toBe(2);
  });

  it("preserva Auction Lab aprobable y excluye tesoreria mutante", () => {
    expect(getTool("manage_auction_lab")?.riskLevel).toBe(2);
    expect(getTool("manage_auction_lab")?.requiredRoles).toEqual(["administracion", "admin"]);
  ...
  it("marca toda persistencia previa a aprobacion como risk 2", () => {
    for (const name of ["create_rfq_draft", "prepare_purchase_order"]) {
      expect(getTool(name)?.riskLevel, name).toBe(2);
    }
    expect(getTool("prepare_email")?.riskLevel).toBe(1);
    expect(getTool("update_spreadsheet_rows")?.riskLevel).toBe(1);
    expect(getTool("update_spreadsheet_rows")?.requiredRoles).toEqual(["administracion", "admin"]);
    expect(getTool("send_email")?.riskLevel).toBe(2);
    expect(getTool("get_finance_overview")?.riskLevel).toBe(0);
    expect(getTool("get_finance_overview")?.requiredRoles).toEqual(["administracion", "admin"]);
  });
});
