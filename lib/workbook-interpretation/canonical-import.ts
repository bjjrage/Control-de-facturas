...
function extractPeriodAndNumber(workbook: WorkbookRepresentation, sheets: WorkbookRepresentation["sheets"]): { periodStart: string | null; periodEnd: string | null; number: number | null } {
  ...
  const period = text.match(/per[ií]odo\s*:\s*desde\s*(\d{1,2})[\/-](\d{1,2})[\/-](\d{2,4})\s*hasta\s*(\d{1,2})[\/-](\d{1,2})[\/-](\d{2,4})/i);
  const normalizeDate = (day: string, month: string, year: string) => {
    const fullYear = year.length === 2 ? `20${year}` : year;
    return `${fullYear}-${month.padStart(2, "0")}-${day.padStart(2, "0")}`;
  };
  const certificate = text.match(/certificado(?:\s+de\s+ejecuci[oó]n\s+de\s+obras)?\s*n[°ºo]?\s*(\d+)/i);
  return {
    periodStart: period ? normalizeDate(period[1], period[2], period[3]) : null,
    periodEnd: period ? normalizeDate(period[4], period[5], period[6]) : null,
    number: certificate ? Number(certificate[1]) : null,
  };
}
