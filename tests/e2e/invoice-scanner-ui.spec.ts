...
test.describe('Invoice Dialog Control Scanner UI & E2E Integration', () => {
  ...
  test('Flujo completo desde UI: Abrir Nueva Factura -> Escanear desde celular -> QR/PIN -> Móvil captura 2 páginas -> Filtros -> Desktop recibe completed -> PDF adjunto en formulario', async ({
    ...
    // 16. Verificar que la UI envía EXCLUSIVAMENTE scanner_session_id y NUNCA campos de storage
    const sessionIdInput = invoiceDialog.locator('input[name="scanner_session_id"]');
    await expect(sessionIdInput).toHaveCount(1);
    const sessionIdValue = await sessionIdInput.inputValue();
    expect(sessionIdValue).toBe(sessionData.sessionId);

    // Los campos de storage ya NO deben existir en el DOM del cliente
    const storagePathInput = invoiceDialog.locator('input[name="scanner_storage_path"]');
    await expect(storagePathInput).toHaveCount(0);
    const fileNameInput = invoiceDialog.locator('input[name="scanner_file_name"]');
    await expect(fileNameInput).toHaveCount(0);
    const fileSizeInput = invoiceDialog.locator('input[name="scanner_file_size"]');
    await expect(fileSizeInput).toHaveCount(0);
