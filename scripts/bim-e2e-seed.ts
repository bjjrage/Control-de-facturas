 ...
 *
 * NO crea el proyecto (lo crea el test vía UI "Nuevo proyecto"), NO crea
 * budget_items (el catálogo entra por el importador Excel REAL desde la UI)
 * y NO inserta nada de bim_* (el IFC entra por la UI).
 * Idempotente: si el usuario/empresa ya existen, los reutiliza (y deja el
...
async function run(): Promise<void> {
  ...
  // NOTA: el proyecto Aurora lo crea el propio test vía UI ("Nuevo proyecto").
  // El seed NO debe pre-crearlo: projects tiene UNIQUE(empresa_id, code) y el
  // diálogo fallaría con "Ya existe un proyecto con el código..." (ese texto
  // contiene el código y rompería las assertions del spec).

  console.log(JSON.stringify({ ok: true, email: TEST_EMAIL, empresa_id: empresaId }));
}
