...
export function ReceiptPortalForm({ token, order }: { token: string; order: ReceiptPortalOrder }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) setError(result.error ?? "No se pudo enviar la recepción.");
    ...
    return (
      <section role="status" className="rounded-lg border border-[var(--border)] bg-[var(--panel)] p-6 text-center">
        ...
        <p className="mt-2 text-sm text-[var(--muted)]">
          La empresa recibió las cantidades y la evidencia. El inventario se actualizará únicamente después de su revisión y confirmación.
        </p>
      </section>
    );
  }
  ...
  return (
    <form onSubmit={submit} className="space-y-4 rounded-lg border border-[var(--border)] bg-[var(--panel)] p-4 sm:p-5">
      ...
      <label className="block text-xs">
        Fotos o evidencia
        <input className="mt-1 block w-full rounded border bg-[var(--panel-2)] p-2 text-sm" type="file" name="files" accept="image/jpeg,image/png,image/webp,image/heic,application/pdf" multiple required />
        <span className="mt-1 block text-[11px] text-[var(--muted)]">Hasta 10 archivos; 20 MB cada uno y 50 MB en total.</span>
      </label>
