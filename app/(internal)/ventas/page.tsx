...
import { Client, SalesDocument, SalesDocStatus } from "@/lib/types";
...
import { docSaldo, isOverdue, SALES_DOC_STATUS_LABELS, SALES_DOC_TYPE_LABELS, SALES_STATUS_ORDER } from "@/lib/sales";
...
export default async function VentasPage({ searchParams }: { searchParams: Promise<Filters> }) {
  await requireModule("ventas", ["administracion", "admin"]);
  const supabase = await createClient();
  const { month: monthParam, q, client: clientId, status } = await searchParams;
  ...
  return (
    <div className="max-w-5xl space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-[17px] font-semibold">Ventas</h1>
      ...
      <form className="rounded-lg border border-[var(--border)] bg-[var(--panel)] p-3 space-y-3" method="get">
        ...
        <div className="flex flex-wrap items-end gap-3">
          <MonthFilter month={month} basePath="/ventas" />
          <div>
      ...
      {groups.length === 0 ? (
        <div className="rounded-lg border border-[var(--border)] bg-[var(--panel)] text-center text-[var(--muted)] py-10 text-[13px]">
          {month ? `No hay ventas con fecha en ${month}.` : "No hay ventas para estos filtros."}
        </div>
      ) : (
        groups.map((g) => (
          <div key={g.status}>
