...
export function LinkOrderDialog({
  ...
  const filtered = q.trim()
    ? candidates.filter((c) => {
        ...
        return c.code.toLowerCase().includes(t) || c.product.toLowerCase().includes(t);
      })
    : candidates;
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent title="Vincular a Orden de Compra">
        <div className="space-y-3">
          ...
          {loading && (
            <p className="text-[13px] text-[var(--muted)] py-6 text-center">Buscando órdenes de compra…</p>
          )}

          {/* Sin OCs del proveedor → no existe OC para esta factura */}
          {!loading && result && candidates.length === 0 && (
            <div className="rounded border border-[var(--border)] bg-[var(--panel-2)] px-3 py-5 text-center space-y-2">
              <p className="text-[13px] text-[var(--muted)]">
                No hay OCs abiertas de <span className="font-medium text-[var(--foreground)]">{result.invoiceProvider}</span>.
              </p>
              <p className="text-[11px] text-[var(--muted)]">
                Esta factura no tiene OC asociada. Podés crear una OC nueva para registrarla.
              </p>
            </div>
          )}
          ...
          {!loading && result && candidates.length > 0 && (
            <>
              ...
              <p className="text-[11px] text-[var(--muted)]">
                {candidates.length} OC{candidates.length !== 1 ? "s" : ""} abiertas de este proveedor
              </p>
            </>
          )}
        </div>
