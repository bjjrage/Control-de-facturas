...
import { DashboardData } from "./section-action";
...
function StatCard({
  ...
  return (
    ...
    >
      <div
        className={`h-11 w-11 rounded-full flex items-center justify-center shrink-0 ${STAT_COLOR_CLASSES[color]}`}
      >
        <Icon size={19} />
      </div>
      <div>
...
export function DashboardSection({ data }: { data: DashboardData }) {
  ...
  if (showRfqStats) {
    stats.push({ label: "Solicitudes abiertas", value: openRfqs, href: "/rfqs?open=1", icon: FileText, color: "primary" });
    stats.push({ label: "Con ofertas para elegir", value: awaitingSelection, href: "/rfqs", icon: Tag, color: "teal" });
  }
  if (showInvoiceStats) {
    stats.push({ label: "Facturas pendientes", value: pending, href: "/invoices?month=all&status=PENDIENTE", icon: FileText, color: "purple" });
    stats.push({ label: "Requieren revisión", value: review, href: "/invoices?month=all&status=REQUIERE_REVISION", icon: AlertCircle, color: "orange" });
    stats.push({ label: "Aptas para pago", value: apto, href: "/invoices?month=all&status=APTO_PARA_PAGO", icon: CheckCircle2, color: "ok" });
  }
  if (showSalesStats) {
    stats.push({ label: "Ventas por cobrar", value: porCobrar, href: "/cobros", icon: Wallet, color: "teal" });
    stats.push({ label: "Ventas vencidas", value: vencidas, href: "/cobros", icon: CalendarClock, color: "orange" });
  }
