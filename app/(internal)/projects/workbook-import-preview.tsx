...
function ResultPreview({ result, candidate }: { result: WorkbookInterpretationResult; candidate: CanonicalImportCandidate }) {
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-emerald-400/20 bg-emerald-400/[0.06] p-3">
        <div className="flex items-center gap-2 text-[13px] font-semibold text-emerald-200"><Sparkles size={15} /> Qu&eacute; entendi&oacute; el ERP</div>
      ...
      <section>
        ...
        <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
          ...
          <Value label="C&oacute;digo / referencia" field={result.project.code} />
          <Value label="Cliente / comitente" field={result.project.client} />
          <Value label="Contratista (solo preview; no se importa)" field={result.project.contractor} />
          <Value label="Ubicaci&oacute;n" field={result.project.location} />
      ...
      <section>
        <h3 className="text-[11px] font-bold uppercase tracking-widest text-[var(--muted)]">Aplicaci&oacute;n can&oacute;nica validada</h3>
        <div className="mt-2 grid gap-2 text-[12px] sm:grid-cols-3">
          <p className="rounded-xl border border-emerald-400/20 bg-emerald-400/[0.06] p-3">Presupuesto<br /><strong>{"\u2713"} {candidate.budgetItems.length} partidas</strong></p>
          <p className="rounded-xl border border-emerald-400/20 bg-emerald-400/[0.06] p-3">Certificado N.&ordm; {candidate.certificate.number ?? "\u2014"}<br /><strong>{candidate.certificate.status === "SAFE_TO_APPLY" ? `${"\u2713"} ${candidate.certificate.matchedBudgetItems}/${candidate.certificate.itemCount} conciliadas` : `${"\u25cb"} no aplicable`}</strong></p>
          <p className="rounded-xl border border-sky-400/20 bg-sky-400/[0.06] p-3">Medici&oacute;n<br /><strong>{candidate.measurement.status === "DETECTED_NOT_APPLIED" ? `${"\u2713"} detectada ${"\u00b7"} ${"\u25cb"} no aplicada` : `${"\u25cb"} no detectada`}</strong></p>
        </div>
        <p className="mt-2 text-[11px] text-[var(--muted)]">Total contractual: <strong>{new Intl.NumberFormat("es-PY", { maximumFractionDigits: 0 }).format(candidate.budgetTotal)}</strong></p>
        {candidate.measurement.status === "DETECTED_NOT_APPLIED" ? <p className="mt-1 text-[11px] text-amber-100">{candidate.measurement.reason}</p> : null}
        {candidate.certificate.status !== "SAFE_TO_APPLY" ? <p className="mt-1 text-[11px] text-amber-100">{candidate.certificate.reason}</p> : null}
      </section>
      ...
      <section>
        <h3 className="text-[11px] font-bold uppercase tracking-widest text-[var(--muted)]">Plan de importaci&oacute;n y cobertura</h3>
        <div className="mt-2 space-y-2">
      ...
      <p className="rounded-lg border border-[var(--border)] bg-white/[0.02] px-3 py-2 text-[11px] text-[var(--muted)]">Este resultado es s&oacute;lo un preview. No se cre&oacute; ninguna obra ni se guardaron datos.</p>
    </div>
  );
}
...
function ResultActions({
  ...
  const nameMissing = result.project.name.status !== "FOUND" || result.project.name.value === null;
  const codeMissing = result.project.code.status !== "FOUND" || result.project.code.value === null;
  const hasBlockingBudget = candidate.budgetBlockingRows.length > 0;
  return (
    <section className="space-y-3 rounded-xl border border-sky-300/20 bg-sky-300/[0.04] p-3">
      <h3 className="text-[11px] font-bold uppercase tracking-widest text-sky-100">Aplicaci&oacute;n al ERP can&oacute;nico</h3>
      <div className="grid gap-2 text-[12px] sm:grid-cols-3">
        <p>Presupuesto: <strong>{candidate.budgetItems.length} partidas</strong> &mdash; aplicable a <code>budget_items</code>.</p>
        <p>Certificado: <strong>{candidate.certificate.status === "SAFE_TO_APPLY" ? `${candidate.certificate.matchedBudgetItems}/${candidate.certificate.itemCount} conciliadas` : candidate.certificate.status}</strong> &mdash; s&oacute;lo se crea si coincide con el presupuesto.</p>
        <p>Medici&oacute;n: <strong>{candidate.measurement.status === "DETECTED_NOT_APPLIED" ? "detectada, no aplicada" : "no detectada"}</strong> &mdash; no se convierte en ejecuci&oacute;n sin correspondencia segura.</p>
      </div>
      ...
      {nameMissing || codeMissing ? (
        <div className="rounded-lg border border-amber-300/25 bg-amber-300/[0.04] p-3">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-amber-100">Completar dato cr&iacute;tico</p>
          <p className="mt-1 text-[11px] text-[var(--muted)]">El archivo no aport&oacute; un identificador suficiente. Complet&aacute; s&oacute;lo lo faltante.</p>
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            ...
            {codeMissing ? <div><Label htmlFor="workbook_code_override">C&oacute;digo de obra</Label><Input id="workbook_code_override" value={codeOverride} onChange={(event) => onCodeOverride(event.target.value)} /></div> : null}
          </div>
        </div>
      ) : null}
      ...
      <div className="flex justify-between gap-2 pt-1">
        ...
        <Button type="button" onClick={onCreate} disabled={creating || hasBlockingBudget || (nameMissing && !nameOverride.trim()) || (codeMissing && !codeOverride.trim())}>
...
export function WorkbookImportPreview({ onBack }: { onBack: () => void }) {
  ...
  async function inspectFile(selected: File | null) {
    ...
    setMetadata({ name: selected.name, size: selected.size, sheets: [] });
  }
  ...
  async function analyze() {
    ...
    try {
      ...
      if (!response.ok || !payload?.result || !payload?.canonical || typeof payload.previewToken !== "string") throw new Error(payload?.error ?? "No se pudo analizar la planilla.");
      const interpreted = payload.result as WorkbookInterpretationResult;
      setResult(interpreted);
      setCandidate(payload.canonical as CanonicalImportCandidate);
      setPreviewToken(payload.previewToken);
      if (metadata && Array.isArray(payload.sheets)) setMetadata({ ...metadata, sheets: payload.sheets.filter((sheet: unknown): sheet is string => typeof sheet === "string") });
      if (interpreted.project.name.status === "FOUND" && interpreted.project.name.value !== null) setNameOverride(String(interpreted.project.name.value));
      if (interpreted.project.code.status === "FOUND" && interpreted.project.code.value !== null) setCodeOverride(String(interpreted.project.code.value));
    } catch (cause) {
  ...
  async function createImportedProject() {
    if (!file || !result || !candidate || !previewToken) return;
    setCreating(true);
    setError(null);
    const formData = new FormData();
    formData.set("file", file);
  ...
  if (result && candidate) return (
    <div className="space-y-4">
      ...
      <ResultActions
        ...
        creating={creating}
        error={error}
        onBack={() => { setResult(null); setCandidate(null); setPreviewToken(null); setError(null); }}
      />
    </div>
  );
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-dashed border-sky-300/25 bg-sky-300/[0.035] p-4 text-center">
        ...
        <p className="mt-2 text-[14px] font-semibold">Sub&iacute; la planilla que ya us&aacute;s</p>
        <p className="mt-1 text-[12px] text-[var(--muted)]">El ERP analizar&aacute; la estructura con OpenAI; se env&iacute;an valores y muestras de la planilla. No necesit&aacute;s adaptar headers ni crear una obra antes.</p>
      ...
      {metadata ? <div className="rounded-xl border border-[var(--border)] bg-white/[0.025] p-3"><p className="text-[13px] font-semibold">{metadata.name}</p><p className="mt-0.5 text-[11px] text-[var(--muted)]">{formatBytes(metadata.size)} {"\u00b7"} {metadata.sheets.length ? `${metadata.sheets.length} ${metadata.sheets.length === 1 ? "hoja" : "hojas"}` : "pendiente de análisis"}</p><div className="mt-2 flex flex-wrap gap-1.5">{metadata.sheets.map((sheet) => <span key={sheet} className="rounded-full border border-[var(--border)] px-2 py-1 text-[10px] text-[var(--muted)]">{sheet}</span>)}</div></div> : null}
      {analyzing ? <div className="flex items-center justify-center gap-2 rounded-xl border border-[var(--border)] py-5 text-[13px] text-[var(--muted)]"><LoaderCircle className="animate-spin" size={16} /> Analizando la estructura de la planilla&hellip;</div> : null}
