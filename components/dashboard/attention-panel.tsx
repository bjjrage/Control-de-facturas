...
export function AttentionPanel({ alerts, title = "Requiere atención" }: { alerts: AttentionAlert[]; title?: string }) {
  ...
  return (
    <div className="rounded-2xl border border-[var(--warn)]/30 bg-[var(--panel)] p-4 shadow-sm sm:p-5">
        ...
        {alerts.map((alert) => {
          ...
          return (
            <Link
              key={alert.id}
              href={alert.href}
              className={`group flex items-center justify-between gap-2.5 rounded-xl border px-3 py-2.5 text-[12px] transition-all duration-150 ${isError ? "border-[var(--error)]/30 bg-[var(--error-bg)]/40 text-[var(--foreground)] hover:bg-[var(--error-bg)]/80" : "border-[var(--warn)]/30 bg-[var(--warn-bg)]/40 text-[var(--foreground)] hover:bg-[var(--warn-bg)]/80"}`}
            >
